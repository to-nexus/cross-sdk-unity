...
namespace Cross.Core.Models.MessageHandler
{
    ...
    public class TypedEventHandler<T, TR> : IDisposable
    {
        ...
        protected virtual async Task RequestCallback(string arg1, JsonRpcRequest<T> arg2)
        {
            ...
            if (isDisposed)
            {
                CrossLogger.Log($"Too late to process request {typeof(T)} in topic {arg1}, the Cross.Core instance {Ref.Context} was disposed before we could");
                return;
            }
            ...
            if (nextIsDisposed)
            {
                CrossLogger.Log($"Too late to send a result for request {typeof(T)} in topic {arg1}, the Cross.Core instance {Ref.Context} was disposed before we could");
                return;
            }
