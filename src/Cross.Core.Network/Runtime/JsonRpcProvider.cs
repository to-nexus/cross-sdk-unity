...
using Cross.Core.Common.Events;
...
namespace Cross.Core.Network
{
    ...
    public class JsonRpcProvider : IJsonRpcProvider
    {
        ...
        public JsonRpcProvider(IJsonRpcConnection connection, string context = null)
        {
            ...
            _logger = CrossLogger.WithContext(Context);
        ...
        public async Task<TR> Request<T, TR>(IRequestArguments<T> requestArgs, object context = null)
        {
            ...
            _jsonResponseEventHolder.OfType<CrossNetworkException>()[request.Id.ToString()] += (sender, exception) =>
            {
                if (requestTask.Task.IsCompleted)
                    return;
        ...
        private void OnPayload(object sender, string json)
        {
            ...
            else
            {
                if (payload.IsError)
                {
                    ...
                    _jsonResponseEventHolder.OfType<CrossNetworkException>()[payload.Id.ToString()](this,
