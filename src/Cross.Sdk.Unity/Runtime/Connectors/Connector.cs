...
namespace Cross.Sdk.Unity
{
    public abstract class Connector
    {
        ...
        public async Task InitializeAsync(CrossSdkConfig config, SignClientUnity signClient)
        {
            if (IsInitialized)
        ...
        protected virtual async Task ApproveSignatureRequestAsync()
        {
            ...
            try
            {
                ...
                var isSignatureValid = await CrossSdk.SiweController.VerifyMessageAsync(new SiweVerifyMessageArgs
                {
                ...
                if (isSignatureValid)
                {
                    _ = await CrossSdk.SiweController.GetSessionAsync(new GetSiweSessionArgs
                    {
                        Address = ethAddress,
                        ChainIds = new[]
                        {
        ...
        protected abstract Task InitializeAsyncCore(CrossSdkConfig config, SignClientUnity signClient);
